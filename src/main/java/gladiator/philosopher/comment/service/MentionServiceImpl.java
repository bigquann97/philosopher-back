...
/*
  ...
  private List<Mention> extractMentions(Comment mentioningComment) {
    List<Mention> mentions = new ArrayList<>();
    String content = mentioningComment.getContent();

    long mentionedCommentId = 1L;
  ...
  private List<Long> parseAllHashtagNums(String body) {
    List<Long> hashtagNums = new ArrayList<>();
    ...
    while (body.contains("#")) {
      ...
      while (body.charAt(pointer) >= 48 && body.charAt(pointer) <= 57) {
        pointer++;
      }
