...
public interface PostRecommendRepository extends JpaRepository<PostRecommend, Long> {
  ...
  @Query("select new gladiator.philosopher.account.dto.PostSimpleResponseDto(pr.id,pr.post.title, pr.post.content, pr.post.category.name) from PostRecommend pr where pr.account.id=:id order by pr.id DESC ")
